...
class PercyLogger {
  ...
  constructor() {
    ...
    _defineProperty(this, "stdout", process.stdout);

    _defineProperty(this, "stderr", process.stderr);
  ...
  log(debug, level, message, meta = {}) {
    let error; // message must be an error object
  ...
  format(message, debug, level, elapsed) {
    ...
    label = _colors.default.magenta(label);

    if (level === 'error') {
      // red errors
      message = _colors.default.red(message);
    } else if (level === 'warn') {
      // yellow warnings
      message = _colors.default.yellow(message);
    } else if (level === 'info' || level === 'debug') {
      // blue info and debug URLs
      message = message.replace(URL_REGEXP, _colors.default.blue('$&'));
    }
