...
class Wait extends _cliCommand.default {
  ...
  progress({
    ...
    let stdout = _logger.default.instance.stdout; // update the same line each time

    _readline.default.cursorTo(stdout, 0); // still recieving snapshots


    if (state === 'pending') {
      stdout.write(_logger.default.format('Recieving snapshots...', 'cli:build:wait')); // need to clear the line before finishing
    } else if (finished === total || state === 'finished') {
      _readline.default.clearLine(stdout);
    } // processing snapshots


    if (state === 'processing') {
      stdout.write(_logger.default.format(`Processing ${count} snapshots - ` + (finished === total ? 'finishing up...' : `${finished} of ${total} comparisons finished...`), 'cli:build:wait'));
    }
  } // Log build status
